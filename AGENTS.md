...
| `src/pricing.rs`        | Embedded LiteLLM price snapshot (`pricing-data.json`) + user override; model-name matching and tiered cost math. |
...
- The pricing snapshot is regenerated by filtering LiteLLM's
  `model_prices_and_context_window.json` to the relevant model families; commit
  the filtered `src/pricing-data.json` (no build-time network — Nix-sandbox
  safe).
