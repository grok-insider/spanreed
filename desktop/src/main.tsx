...
import { ApiKeyForm, ProviderCard, WorkspaceShell, type ProviderOutput, type SharingConsent } from "@fabrials/ui";
...
function App() {
  ...
  const [mode, setMode] = React.useState(() => localStorage.getItem("spanreed.mode"));
  ...
  async function chooseLocal() { localStorage.setItem("spanreed.mode","local");setMode("local"); }
  async function openHosted() {
    try { await invoke("open_hosted");setNotice("Continue in your browser to connect to Fabrials. Local credentials have not been transferred."); }
    catch(error) { setError(String(error)); }
  }
  ...
  if (!mode) return <WorkspaceShell title="Spanreed" navigation={[]} active="" onNavigate={()=>{}}><div className="fb-heading"><p>WELCOME TO FABRIALS</p><h1>Where do you want to run?</h1><p>One place for your AI accounts, usage and limits.</p></div><div className="fb-grid">
    <article className="fb-card"><header><h2>On your machine</h2><p className="fb-muted">Your credentials and history stay here. No Fabrials account required.</p></header><div className="fb-card-body"><button className="fb-button fb-button-primary" onClick={chooseLocal}>Use Spanreed locally</button></div></article>
    <article className="fb-card"><header><h2>Hosted by Fabrials</h2><p className="fb-muted">Use ai-relay in your browser. Sharing metrics and connecting provider accounts are separate choices.</p></header><div className="fb-card-body"><button className="fb-button" onClick={openHosted}>Open ai-relay</button></div></article>
  </div>{notice && <p role="status">{notice}</p>}{error && <p className="fb-error" role="alert">{error}</p>}</WorkspaceShell>;
  return <WorkspaceShell title="Spanreed" navigation={navigation} active={page} onNavigate={setPage} actions={<div className="fb-row"><span className="fb-muted">Local workspace</span><button className="fb-button" disabled={busy} onClick={()=>void load(true)}>{busy?"Refreshing…":"Refresh"}</button></div>}>
    ...
    {page === "accounts" && <><div className="fb-heading"><h1>Accounts</h1><p>Managed accounts on this machine. CLI credentials also appear in Overview.</p></div>{reauthorize && (reauthorize.provider === "grok" || reauthorize.provider === "nous") && <section><DeviceLogin key={reauthorize.id} provider={reauthorize.provider} accountId={reauthorize.id} initialAlias={reauthorize.alias} onConnected={()=>{setReauthorize(null);void load();}} /><button className="fb-button" onClick={()=>setReauthorize(null)}>Close authorization</button></section>}<div className="fb-grid"><DeviceLogin provider="grok" onConnected={()=>void load()} /><DeviceLogin provider="nous" onConnected={()=>void load()} /><ApiKeyForm accounts={accounts} onSave={async input => { await invoke("add_api_key", {...input}); await load(); }} /></div><div className="fb-grid">{accounts.map(account=><article className="fb-card" key={account.id}><header><h2>{account.alias}</h2><p className="fb-muted">{account.provider} · {account.plan_label || "Plan not reported"}</p></header><div className="fb-card-body"><button className="fb-button" disabled={busy || account.active} onClick={async()=>{try{await invoke("activate_account",{id:account.id});await load();}catch(error){setError(String(error));}}}>{account.active?"Active account":"Make active"}</button>{(account.provider === "grok" || account.provider === "nous") && <button className="fb-button" disabled={busy} onClick={()=>setReauthorize(account)}>Authorize again</button>}<AccountActions key={`${account.id}:${account.generation ?? "legacy"}`} account={account} onChanged={()=>load()} /></div></article>)}</div>{!accounts.length&&<div className="fb-empty"><p>No managed accounts. Existing CLI logins remain available to usage detection.</p></div>}</>}
    ...
    {page === "settings" && <><div className="fb-heading"><h1>Workspace settings</h1><p>Choose what leaves this machine.</p></div><div className="fb-form"><label>Appearance<select value={theme} onChange={event=>setTheme(event.target.value)}><option value="system">System</option><option value="light">Light</option><option value="dark">Dark</option></select></label>
      ...
      <button className="fb-button" onClick={openHosted}>Open hosted ai-relay</button>
    </div></>}
  </WorkspaceShell>;
}
