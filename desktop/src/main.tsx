...
function App() {
  ...
  const workspacePicker = <label>Workspace<select aria-label="Workspace" value={mode} onChange={event => changeMode(event.target.value)}><option value="local">On this machine</option><option value="remote">ai-relay · Fabrials</option></select></label>;
  if (mode === "remote") return <WorkspaceShell title="Spanreed" navigation={remoteNavigation} active={page} onNavigate={setPage} actions={workspacePicker}><RemoteWorkspace page={page}/></WorkspaceShell>;
  return <WorkspaceShell title="Spanreed" navigation={navigation} active={page} onNavigate={setPage} actions={<div className="fb-row">{workspacePicker}<button className="fb-button" disabled={busy} onClick={()=>void load(true)}>{busy?"Refreshing…":"Refresh"}</button></div>}>
    ...
    {page === "settings" && <><div className="fb-heading"><h1>Workspace settings</h1><p>Choose what leaves this machine.</p></div><div className="fb-form"><label>Appearance<select value={theme} onChange={event=>setTheme(event.target.value)}><option value="system">System</option><option value="light">Light</option><option value="dark">Dark</option></select></label>
      <FabrialsLink onChange={view=>setLinkIdentity(view.state === "linked" ? view.user.id : view.state)}/>
      ...
      <SharingControls key={linkIdentity} sources={[...outputs.map(output=>output.providerId),...accounts.map(account=>account.id)]}/>
