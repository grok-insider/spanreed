...
interface Catalog { clients: {id: string; name: string; remote_collection: boolean}[]; settings: Settings }
...
export function LocalUsage() {
  ...
  const refresh = React.useCallback(async (force = false) => {
    ...
    try {
      const result = await invoke<ConsumptionReport>("usage_report",{force,filter:{client:client || null,model:model.trim() || null,since_ms:Date.now()-days*86400000}});
  ...
  return <>
    ...
    <details className="fb-card"><summary className="fb-card-body">Connect usage reports</summary><form className="fb-card-body fb-form" onSubmit={event=>{
      ...
      void invoke("connect_usage_source",{connection:{client:remoteClient,account,credential}})
        .then(async()=>{setCredential("");setNotice("Usage connection saved on this machine.");await refresh(true);})
        .catch(error=>setError(String(error))).finally(()=>setBusy(false));
    }}>
      ...
      <button type="button" className="fb-button" disabled={busy} onClick={()=>{
        setBusy(true);void invoke("disconnect_usage_source",{client:remoteClient}).then(async()=>{setCredential("");setNotice("Connection and imported report cleared.");await refresh(true);}).catch(error=>setError(String(error))).finally(()=>setBusy(false));
    ...
    <details className="fb-card"><summary className="fb-card-body">Additional source folders</summary><div className="fb-card-body fb-form">
      ...
      <button className="fb-button" disabled={busy || !catalog} onClick={()=>void saveRoots()}>Save folders</button>{notice && <p role="status">{notice}</p>}
    </div></details>
  </>;
}
