...
export function SharingControls({sources,enabled,connected}:{sources:string[];enabled:boolean;connected:boolean}) {
  ...
  React.useEffect(()=>{void invoke<{clients:{id:string}[]}>("usage_sources").then(value=>setClients(value.clients.map(client=>client.id))).catch(error=>setError(String(error)));},[]);
  const choices=[...new Set([...sources,...clients,...selection.sources])].sort();
  return <><section className="fb-card"><header><h2>Private synchronization</h2><p className="fb-muted">Choose exactly which local sources may upload quotas, local daily usage and available request metadata. Provider credentials and request bodies stay private.</p></header><div className="fb-card-body fb-form">
    <div className="fb-source-selection">{choices.map(source=><label key={source}><span><input type="checkbox" disabled={busy} checked={selection.sources.includes(source)} onChange={event=>setSelection({sources:event.target.checked?[...selection.sources,source]:selection.sources.filter(value=>value!==source)})}/>{source}</span></label>)}</div>
