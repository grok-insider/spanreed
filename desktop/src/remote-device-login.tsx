...
export function RemoteDeviceLogin({ provider, initialAlias, reauthorize = false, migration, onConnected }: {provider:"grok" | "nous"; initialAlias?:string; reauthorize?:boolean; migration?:{migrationId:string;sourceId:string}; onConnected:() => Promise<void>}) {
  ...
  async function cancelRemote(flow:Flow) { await remote(provider === "grok" ? "cancelGrok" : "cancelNous", provider === "grok" ? {state:flow.id} : {id:flow.id}); }
  ...
  React.useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        const result = provider === "grok" ? await remote<GrokDeviceProgress>("pollGrok",{state:flow!.id}) : await remote<Progress>("pollNous",{id:flow!.id});
        if (stopped) return;
  ...
  async function begin(event:React.FormEvent) {
    ...
    try {
      ...
      if (provider === "grok") {
        ...
        const value = await remote<LoginView>(reauthorize ? "reauthorizeNous" : "beginNous",{alias,...migration});
  ...
  return <section className="fb-card"><header><h2>{reauthorize ? "Renew" : "Connect"} {provider === "grok" ? "SuperGrok" : "Nous"} on ai-relay</h2><p className="fb-muted">Provider credentials will be held by your hosted workspace.</p></header><div className="fb-card-body fb-form">
