...
export function SynchronizedConsumption({load}: {load:()=>Promise<{snapshots:ConsumptionSnapshot[]}>}) {
  ...
  React.useEffect(()=>{let active=true;void load().then(result=>{if(active){setSnapshots(result.snapshots);setDevice(result.snapshots[0]?.device || "");}}).catch(error=>{if(active)setError(String(error));}).finally(()=>{if(active)setBusy(false);});return()=>{active=false;};},[load]);
  return <>
