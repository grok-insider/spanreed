...
- Account quota and resets: the authenticated ChatGPT usage/reset endpoints, through `crates/spanreed-adapters/src/providers/codex.rs` and the reset inventory helpers.
- Local tokens and estimated API-equivalent cost: Codex rollout JSONL, through `crates/spanreed-adapters/src/cost.rs`. This does not require routing requests through ai-relay.
