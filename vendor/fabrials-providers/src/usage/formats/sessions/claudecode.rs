...
use crate::usage::formats::{pricing, provider_identity, TokenBreakdown};
...
pub fn parse_claude_file(path: &Path) -> Vec<UnifiedMessage> {
    let home_dir = crate::usage::formats::paths::home_dir();
...
fn cc_mirror_variant_metadata_from_path(
    ...
    let variant_dir =
        crate::usage::formats::cc_mirror::variant_dir_from_session_path(path, home_dir)?;
    let variant_name = variant_dir.file_name()?.to_string_lossy().to_string();
    let variant_path = crate::usage::formats::cc_mirror::variant_file_path(&variant_dir);
    let metadata = crate::usage::formats::cc_mirror::read_variant_file(&variant_path);
...
mod tests {
    use super::*;
    use crate::usage::formats::paths::json_path_literal;
    use std::io::Write;
