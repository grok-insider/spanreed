...
pub trait HopObserver {
    ...
    /// Whether a model named inside a tunneled request may run on this
    /// credential. Hosts must decide; there is no permissive default.
...
pub fn forward(hop: AuthorizedHop<'_>, observer: &dyn HopObserver) -> Result<(), String> {
    ...
    let client_http = reqwest::blocking::Client::builder()
        ...
        .user_agent("fabrials-fabric/0.1")
        .build()
    ...
    let attempt_result = loop {
        ...
        if prov.rewrites_models_list(&method, &routed) {
...
/// Headers a client may use to present its own credential. They never reach
/// an upstream that receives an injected credential.
pub const CLIENT_CREDENTIAL_HEADERS: &[&str] = &["authorization", "x-xai-token-auth"];
...
mod tests {
    use super::*;
    use crate::provider::{BodyShaper, CredentialInjector, Router, Translator, UsageExtractor};
    ...
    impl CredentialInjector for FakeProvider {
