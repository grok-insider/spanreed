...
use fabrials_fabric::ports::UsageStore;
pub use fabrials_fabric::ports::{ImportBatch, StoredSource};
use fabrials_types::consumption::{ConsumptionRecord, UsageFilter};
use rusqlite::{params, Connection, OptionalExtension};
use std::path::Path;

pub struct SqliteUsageStore(Connection);
...
mod tests {
    use super::*;
    use fabrials_types::consumption::{Granularity, ImportCheckpoint, Tokens, UsageOrigin};
    ...
    fn apply(store: &mut SqliteUsageStore, source: &str, records: &[ConsumptionRecord]) -> u64 {
    ...
    fn rebuilds_replace_totals_and_clear_to_zero() {
        let mut store =
            SqliteUsageStore::initialize(Connection::open_in_memory().unwrap()).unwrap();
