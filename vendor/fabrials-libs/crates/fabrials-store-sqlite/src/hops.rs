...
use fabrials_fabric::ports::HopStore;
use fabrials_types::HopRecord;
use rusqlite::{params, OptionalExtension};
use std::path::Path;

pub struct SqliteHopStore {
...
impl SqliteHopStore {
    ...
    pub fn append(&mut self, namespace: &str, record: &HopRecord) -> Result<bool, String> {
        validate_namespace(namespace)?;
        let identity = record_identity(record).unwrap_or_else(|| {
            format!(
                "generated:{}",
...
impl HopStore for SqliteHopStore {
    ...
    fn read_window(
        ...
        SqliteHopStore::read_window(self, namespace, provider, from_ms, to_ms)
    }
}
...
mod tests {
    ...
    impl Fixture {
        ...
        fn store(&self) -> SqliteHopStore {
            SqliteHopStore::open(&self.0.join("runtime.sqlite3")).unwrap()
        }
    }
