...
use fabrials_fabric::ports::DeliveryStore;
use rusqlite::params;
use std::path::Path;

pub struct SqliteDeliveryStore(rusqlite::Connection);
pub struct Claim {
    namespace: String,
    key: String,
    token: String,
...
impl SqliteDeliveryStore {
    ...
    pub fn claim(
        ...
        let token = fabrials_fabric::accounting::new_request_id();
...
impl DeliveryStore for SqliteDeliveryStore {
    ...
    fn claim(
        ...
        SqliteDeliveryStore::claim(self, namespace, key, now, expires)
    }
    fn acknowledge(&self, claim: &Claim) -> Result<(), String> {
        SqliteDeliveryStore::acknowledge(self, claim)
    }
}
...
mod tests {
    ...
    fn claims_survive_restart_and_retry_only_without_acknowledgement() {
        ...
        let first = SqliteDeliveryStore::open(&path).unwrap();
        let second = SqliteDeliveryStore::open(&path).unwrap();
