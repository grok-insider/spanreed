...
use super::utils::{open_readonly_sqlite_opt, sqlite_for_each_row_on, SqliteScan};
use super::{
...
/// Full scan of every query group.
///
/// `prepared()` selects the variant -- it is the schema probe, and a variant
/// that prepared is the one this database understands, so falling through to
/// an older query would read the wrong columns.
