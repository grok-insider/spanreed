...
//!
//! Requires `spanreed share login` (same Fabrials X JWT). This install has one
//! Grok CLI identity (`~/.grok/auth.json`).

use crate::app;
use crate::grok_ledger::{self, UsageRecord};
...
/// Best-effort after a Grok probe: push the CLI account + recent ledger, pull hops.
pub fn after_probe() {
    if crate::app::env_offline() {
        return;
    }
    ...
    if args.iter().any(|a| a == "-h" || a == "--help") {
        println!(
            "spanreed sync — push/pull SuperGrok plan + capture hops to ai.fabrials.com\n\n\
             ...
             Pushes this install's Grok CLI identity (plan, pool %, request ledger).\n\
...
pub fn run(_verbose: bool) -> Result<String, String> {
    ...
    let grok_tok =
        providers::grok::current_access_token().ok_or("no grok token — run `grok login`")?;
    ...
    let base = relay_base();
    let _id = put_account(&base, &token, &material)?;
    let recs = grok_ledger::read_window(util::now_ms());
    let mut pushed = 0usize;
    for chunk in recs.chunks(BATCH) {
        pushed += post_usage(&base, &token, chunk)?;
    }
    let pulled = pull_into_local(&base, &token, &material)?;
    Ok(format!(
        "synced {ACCOUNT_ID} (pushed {pushed} hops, pulled {pulled})"
    ))
}

fn put_account(base: &str, access: &str, material: &str) -> Result<String, String> {
...
fn put_json(base: &str, access: &str, path: &str, body: &str) -> Result<String, String> {
    ...
    let res = crate::http::Request::put(url)
...
fn post_usage(base: &str, access: &str, recs: &[UsageRecord]) -> Result<usize, String> {
    ...
    let payload: Vec<serde_json::Value> = recs
        .iter()
        .map(|r| {
            let mut v = serde_json::to_value(r).unwrap_or_else(|_| serde_json::json!({}));
            if let Some(obj) = v.as_object_mut() {
                obj.entry("account_id")
                    .or_insert_with(|| serde_json::json!(ACCOUNT_ID));
            }
            v
        })
        .collect();
    let body = serde_json::json!({ "records": payload });
...
fn pull_into_local(base: &str, access: &str, material: &str) -> Result<usize, String> {
