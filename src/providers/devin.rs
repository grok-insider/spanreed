...
impl Provider for Devin {
    ...
    fn probe(&self) -> ProviderOutput {
        ...
        let api_key = match toml_string(&text, "windsurf_api_key") {
            Some(k) => k,
            None => return ProviderOutput::error(ID, NAME, "Devin credentials missing windsurf_api_key."),
        };
        ...
        if !(200..300).contains(&resp.status) {
            return ProviderOutput::error(ID, NAME, format!("status request failed (HTTP {})", resp.status));
        }
        let data = match resp.json() {
            Some(d) => d,
            None => return ProviderOutput::error(ID, NAME, "Devin quota data unavailable. Try again later."),
        };
            ...
            .and_then(|v| v.as_f64())
        {
            if micros > 0.0 {
                lines.push(MetricLine::Text {
                    ...
                    value: format!("${:.2}", micros / 1_000_000.0),
                    color: None,
                    subtitle: None,
                });
            }
        ...
        if lines.is_empty() {
            return ProviderOutput::error(ID, NAME, "Devin quota data unavailable. Try again later.");
        }
