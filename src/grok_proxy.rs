...
fn record_usage_from_capture(
    ...
    let mut rec = partial.into_record(
        util::now_ms(),
        session_id,
        account_id,
        Some(meta.route.into()),
    );
    rec.duration_ms = Some(meta.elapsed.as_millis() as u64);
    rec.status = Some(meta.status);
    rec.kind = meta.kind.map(str::to_string);
