...
fn cmd_capture(args: &[String]) -> ExitCode {
    match args.first().map(String::as_str) {
        Some("serve") | None => {
            ...
            let options = match capture::Options::parse(rest) {
                Ok(options) => options,
                Err(error) => {
            ...
            capture_log::append(&format!("capture serve integrated bind={}", options.bind));
            match options.serve() {
                Ok(()) => ExitCode::SUCCESS,
                Err(error) => {
                    eprintln!("capture: {error}");
                    capture_log::append(&format!("capture error: {error}"));
                    ExitCode::FAILURE
                }
            }
        }
