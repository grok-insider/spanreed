...
//! `crate::formats::scanner`. Session identity for dedup is derived from in-file
...
use crate::formats::provider_identity::inferred_provider_from_model;
    ...
    loop {
        ...
        if let Ok(entry) = simd_json::from_slice::<CodexEntry>(&mut buffer) {
            if let Some(payload) = entry.payload {
                ...
                if entry.entry_type == "turn_context" {
                    state.current_model = payload_model.clone();
                    let turn_start_ms = (entry.timestamp.as_deref()).and_then(rfc3339_ms);
...
mod tests {
    ...
    #[test]
    fn test_parse_reader_stops_on_line_read_error() {
        ...
        let parsed = parse_codex_reader(reader, "session", 0, CodexParseState::default());
    ...
    fn test_user_message_without_turn_context_anchors_at_user_message() {
        ...
        assert_eq!(
            ...
            (Some("1970-01-01T01:00:00Z")).and_then(rfc3339_ms).unwrap(),
