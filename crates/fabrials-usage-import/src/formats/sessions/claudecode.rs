...
use super::utils::duration_between_ms;
use super::utils::rfc3339_ms;
use super::utils::{
...
use crate::formats::{pricing, provider_identity, TokenBreakdown};
...
pub fn parse_claude_file(path: &Path) -> Vec<UnifiedMessage> {
    let home_dir = crate::formats::paths::home_dir();
    ...
    for line in reader.lines() {
        ...
        if let Ok(entry) = simd_json::from_slice::<ClaudeEntry>(&mut buffer) {
            ...
            if entry.entry_type == "user" || entry.entry_type == "tool_result" {
                ...
                if let Some(timestamp_ms) = (entry.timestamp.as_deref()).and_then(rfc3339_ms) {
            ...
            if entry.entry_type == "assistant" {
                    ...
                    (Some(msg_id), Some(req_id)) => {
                        ...
                        if let Some(&existing_idx) = processed_hashes.get(&hash) {
                            merge_claude_duplicate(
                                &mut messages[existing_idx],
                                &usage,
                                (entry.timestamp.as_deref()).and_then(rfc3339_ms),
                            );
                ...
                let parsed_timestamp = (entry.timestamp.as_deref()).and_then(rfc3339_ms);
...
fn cc_mirror_variant_metadata_from_path(
    ...
    let variant_dir = crate::formats::cc_mirror::variant_dir_from_session_path(path, home_dir)?;
    let variant_name = variant_dir.file_name()?.to_string_lossy().to_string();
    let variant_path = crate::formats::cc_mirror::variant_file_path(&variant_dir);
    let metadata = crate::formats::cc_mirror::read_variant_file(&variant_path);
...
fn extract_claude_tool_result_message(
    ...
    let model = canonicalize_claude_model(&raw_model);
    let timestamp = (context.entry.timestamp.as_deref())
        .and_then(rfc3339_ms)
...
mod tests {
    use super::*;
    use crate::formats::paths::json_path_literal;
    use std::io::Write;
