    ...
    if options.lossy_line_reader
        ...
        && (entry.has_damaged_timestamp()
            || entry
                .timestamp
                .as_deref()
                .is_some_and(|timestamp| rfc3339_ms(timestamp).is_none()))
    {
        return None;
    }
...
fn parse_pi_format_file_inner(
    ...
    for_each_json_line_with_bytes(path, &mut |line| {
        ...
        let recorded_timestamp = entry.timestamp.as_deref().and_then(rfc3339_ms);
...
mod tests {
    ...
    fn utf8_bom_before_the_header_keeps_the_transcript() {
        ...
        for messages in [
            ...
            crate::formats::sessions::senpi::parse_senpi_file(file.path()),
            crate::formats::sessions::kimchi::parse_kimchi_file(file.path()),
