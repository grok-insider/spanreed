...
/// Epoch milliseconds for an RFC 3339 timestamp; the one strict date parser
/// every reader shares.
...
/// Positive span between two instants, `None` when either is missing or the
/// span is not positive.
pub(crate) fn duration_between_ms(start_ms: Option<i64>, end_ms: Option<i64>) -> Option<i64> {
    let duration = end_ms?.saturating_sub(start_ms?);
    (duration > 0).then_some(duration)
}

/// Lossy line reader over a file, or `None` when it cannot be opened.
pub(crate) fn open_lossy_lines(
    path: &Path,
) -> Option<LossyLines<std::io::BufReader<std::fs::File>>> {
    std::fs::File::open(path)
        .ok()
        .map(|file| lossy_lines(std::io::BufReader::new(file)))
}

pub(crate) fn parse_timestamp_str(value: &str) -> Option<i64> {
    if let Some(ms) = rfc3339_ms(value) {
        return Some(ms);
    }
...
pub(crate) fn resolved_provider(
    ...
    provider
        ...
        .and_then(|provider| crate::formats::provider_identity::canonical_provider(provider.trim()))
        .or_else(|| {
            crate::formats::provider_identity::inferred_provider_from_model(model_id)
                .map(str::to_string)
        })
