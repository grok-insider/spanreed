//! Transactional SQLite quota history.
use fabrials_fabric::history::prepare_sample;
use fabrials_fabric::ports::HistoryStore;
...
impl HistoryStore for SqliteHistoryStore {
    fn append(&mut self, namespace: &str, samples: &[HistorySample]) -> Result<usize, String> {
        SqliteHistoryStore::append(self, namespace, samples)
    }
    fn read(
        &self,
        namespace: &str,
        ...
        SqliteHistoryStore::read(self, namespace, provider, limit)
    }
    fn prune(&mut self, namespace: &str, cutoff_ms: i64) -> Result<usize, String> {
        SqliteHistoryStore::prune(self, namespace, cutoff_ms)
    }
}
...
mod store_tests {
    ...
    fn independent_connections_deduplicate_and_isolate_namespaces() {
        ...
        drop(SqliteHistoryStore::open(&fixture.path()).unwrap());
        ...
        let mut store = SqliteHistoryStore::open(&fixture.path()).unwrap();
    ...
    fn reset_events_survive_reopen_and_retention_keeps_last_observation() {
        ...
        drop(store);
        let store = SqliteHistoryStore::open(&fixture.path()).unwrap();
