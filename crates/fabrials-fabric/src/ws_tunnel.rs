//! Byte-pipe WebSocket upgrade to a voice upstream (STT / TTS stream /
//! realtime). Does not parse frames. TLS terminates in front; this hop is
//! plaintext to the client and TLS to the upstream.
...
fn build_handshake(
    ...
    for (k, v) in client_headers {
        ...
        if !k.eq_ignore_ascii_case("authorization")
            && crate::forward::is_client_credential_header(k)
        {
            continue;
        }
