...
pub trait ConnectionHost: Send + Sync + 'static {
    ...
    /// Where this host serves its control plane; health probes are admitted
    /// under it even when the request plane is saturated.
    fn control_prefix(&self) -> crate::routes::ControlPrefix {
        crate::routes::ControlPrefix::default()
    }
    ...
    if health_only
        && !(head.method.as_deref() == Some("GET")
            && head
                .path
                .as_deref()
                .is_some_and(|path| cfg.control_prefix().is_health(path)))
    {
