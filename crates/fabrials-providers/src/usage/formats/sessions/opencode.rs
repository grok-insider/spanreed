...
use crate::usage::formats::{provider_identity, TokenBreakdown};
#[cfg(test)]
...
fn migration_cache_dir() -> std::path::PathBuf {
    crate::usage::formats::paths::get_cache_dir()
}
...
fn legacy_migration_cache_paths() -> Vec<std::path::PathBuf> {
    if crate::usage::formats::paths::is_config_dir_overridden() {
        return Vec::new();
    }

    [
        crate::usage::formats::paths::legacy_dirs_cache_dir()
            .map(|d| d.join(MIGRATION_CACHE_FILENAME)),
        crate::usage::formats::paths::legacy_dot_cache_tokscale_dir()
            .map(|d| d.join(MIGRATION_CACHE_FILENAME)),
    ]
    .into_iter()
    .flatten()
...
pub fn save_opencode_migration_cache(cache: &OpenCodeMigrationCache) {
    ...
    let result = (|| -> std::io::Result<()> {
        ...
        file.sync_all()?;
        crate::usage::formats::fs_atomic::replace_file(&tmp_path, &final_path)?;
        Ok(())
    })();
...
mod tests {
    use super::*;
    use crate::usage::formats::paths::test_env::EnvGuard;
    ...
    fn test_parse_v2_session_message_reads_tokens_and_workspace() {
        ...
        assert_eq!(
            msg.cost_source,
            crate::usage::formats::sessions::CostSource::ProviderReported
        );
    }
    #[test]
    ...
    fn test_parse_opencode_file_keeps_zero_cost_unknown_for_estimation() {
        ...
        assert_eq!(
            msg.cost_source,
            crate::usage::formats::sessions::CostSource::Unknown,
    ...
    fn migration_record_falls_back_to_legacy_path() {
        ...
        let legacy_path = crate::usage::formats::paths::legacy_dirs_cache_dir()
            .unwrap()
