...
pub fn economics_from_hops(records: &[HopRecord], w: &HopsWindow) -> Option<ProviderEconomics> {
    ...
    let (tok_obs, usd_obs, models, observed_window, partial) =
        if let Some(week_start) = w.week_start_ms {
            ...
            let first = recs
                ...
                .min();
            let late =
                !w.complete_ledger && first.is_some_and(|f| f > week_start.saturating_add(DAY_MS));
            if t > 0 || u > 0.0 {
                (t, u, m, Some("since_weekly_reset".into()), late)
            } else {
                (tok_30, usd_30, models_30, Some("last_30d".into()), true)
            }
        } else {
